...
import ArticleModal from '@/app/components/admin-dashboard/ArticleModal'
import { PencilIcon, PlusIcon, TrashIcon, EyeIcon, EyeSlashIcon, DocumentTextIcon } from '@heroicons/react/24/outline'
...
export default function ArticlePage() {
  ...
  const fetchArticles = async () => {
    try {
      setLoading(true)
      const token = localStorage.getItem('token') || ''
      const res = await fetch('/api/article', { headers: { Authorization: `Bearer ${token}` } })
      if (!res.ok) throw new Error('Gagal memuat artikel')
      const data = await res.json()
      setItems(data)
    } catch (e) {
      SweetAlerts.error.simple('Gagal Memuat', 'Tidak dapat memuat daftar artikel')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleOpenEdit = (item: ArticleItem) => { setSelected(item); setIsModalOpen(true) }
  const handleCloseModal = () => setIsModalOpen(false)
  const handleSuccess = () => fetchArticles()

  const handleDelete = async (id: number) => {
    const confirm = await SweetAlerts.confirm.delete('Artikel ini')
    if (!confirm.isConfirmed) return
    try {
      const token = localStorage.getItem('token') || ''
      const res = await fetch(`/api/article/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
      if (!res.ok) throw new Error('Gagal menghapus')
      SweetAlerts.toast.success('Artikel dihapus')
      fetchArticles()
    } catch {
      SweetAlerts.error.simple('Gagal Menghapus', 'Tidak dapat menghapus artikel')
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 ml-0 lg:ml-64">
          <div className="mb-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-2">Kelola artikel yang akan ditampilkan di website</p>
                <div className="flex items-center gap-4 mt-4">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
                    <span className="text-sm text-gray-600">{activeCount} artikel aktif</span>
                  </div>
                  <div className="text-sm text-gray-500">Total {items.length} artikel</div>
                </div>
              </div>
              <button onClick={handleOpenCreate} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700">
                <PlusIcon className="w-5 h-5" /> Tambah Artikel
              </button>
            </div>
          </div>
          ...
          <ArticleModal isOpen={isModalOpen} onClose={handleCloseModal} article={selected} onSuccess={handleSuccess} />
        </main>
      </div>
    </div>
  )
