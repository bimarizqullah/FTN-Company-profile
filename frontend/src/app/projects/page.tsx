...
const Projects: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'ongoing' | 'pending' | 'terminated'>('all');
  ...
  const filterButtons = [
    ...
    { key: 'terminated', label: 'Completed', count: projects.filter(p => p.status === 'terminated').length }
  ];
              ...
              <button
                ...
                className={`inline-flex items-center gap-3 px-6 py-3 rounded-2xl font-semibold transition-all duration-300 transform hover:scale-105 ${
                  filter === btn.key
                    ...
                    : 'bg-white/80 backdrop-blur-sm text-gray-600 hover:text-blue-600 shadow-md hover:shadow-lg border border-gray-200/50'
                }`}
              >
                <span>{btn.label}</span>
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-full border backdrop-blur-sm font-medium text-xs uppercase tracking-wider ${getStatusColor(project.status)}`}>
                ...
                {/* Corner Accent */}
                <div className="absolute bottom-0 left-0 w-16 h-16 bg-gradient-to-tr from-blue-500/10 to-transparent rounded-tr-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              </div>
            ))}
          </div>
          ...
          {filteredProjects.length === 0 && !error && (
            <div className="text-center py-20" data-aos="fade-up">
              ...
              <p className="text-gray-600 max-w-md mx-auto">
                {filter === 'all' 
                  ? 'Our project portfolio is currently being updated. Please check back soon.'
                  : `There are currently no ${filter} projects to display. Try selecting a different filter.`
                }
              </p>
            </div>
          )}
            ...
            <div className="text-center">
              <div className="relative inline-block p-6 mb-4">
                ...
                <div className="relative">
                  <div className="text-4xl lg:text-5xl font-black text-white mb-2">{projects.length}</div>
                  <div className="text-blue-100 font-semibold uppercase tracking-wider text-sm">Total Projects</div>
                </div>
              </div>
            </div>
            <div className="text-center">
              <div className="relative inline-block p-6 mb-4">
                <div className="absolute inset-0 bg-white/10 rounded-2xl backdrop-blur-sm"></div>
                <div className="relative">
                  <div className="text-4xl lg:text-5xl font-black text-white mb-2">{projects.filter(p => p.status === 'ongoing').length}</div>
                  <div className="text-blue-100 font-semibold uppercase tracking-wider text-sm">Active Projects</div>
                </div>
              </div>
            </div>
            <div className="text-center">
              <div className="relative inline-block p-6 mb-4">
                <div className="absolute inset-0 bg-white/10 rounded-2xl backdrop-blur-sm"></div>
                <div className="relative">
                  <div className="text-4xl lg:text-5xl font-black text-white mb-2">{projects.filter(p => p.status === 'terminated').length}</div>
                  <div className="text-blue-100 font-semibold uppercase tracking-wider text-sm">Completed</div>
                </div>
              </div>
            </div>
            <div className="text-center">
              <div className="relative inline-block p-6 mb-4">
                <div className="absolute inset-0 bg-white/10 rounded-2xl backdrop-blur-sm"></div>
                <div className="relative">
                  <div className="text-4xl lg:text-5xl font-black text-white mb-2">{new Set(projects.map(p => p.location)).size}</div>
                  <div className="text-blue-100 font-semibold uppercase tracking-wider text-sm">Locations</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-gradient-to-br from-gray-50 to-blue-50/50 rounded-3xl p-12 lg:p-16 shadow-xl relative overflow-hidden" data-aos="fade-up">
            ...
            <div className="absolute inset-0 opacity-5">
              ...
              <div className="absolute top-1/2 left-1/2 w-24 h-24 bg-blue-500 rounded-full -translate-x-1/2 -translate-y-1/2"></div>
            </div>
