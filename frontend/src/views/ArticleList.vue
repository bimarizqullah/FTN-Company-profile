...
// ✅ Fetch categories dengan filter type='article'
const fetchCategories = async () => {
  try {
    categories.value = await categoryService.getCategories('article')
  } catch (error) {
    console.error('Error fetching categories:', error)
  }
}

// ✅ Fetch subcategories dengan filter type='article'
const fetchSubCategories = async (categoryId?: number) => {
  try {
    subCategories.value = await categoryService.getSubCategories(categoryId, 'article')
  } catch (error) {
...
onMounted(async () => {
  loading.value = true
  const [articles, cats, subs] = await Promise.all([
    articleService.getArticle(),
    categoryService.getCategories('article'),
    categoryService.getSubCategories(undefined, 'article')
  ])
...
// Filtered articles based on selected category/subcategory
const filteredArticles = computed(() => {
  let filtered = items.value

  if (selectedCategoryId.value) {
    filtered = filtered.filter(a => a.categoryId === selectedCategoryId.value)
  }

  if (selectedSubCategoryId.value) {
    filtered = filtered.filter(a => a.subCategoryId === selectedSubCategoryId.value)
  }
...
const groupedArticles = computed(() => {
  const grouped: Record<string, { category: Category | null; subCategories: Record<string, { subCategory: SubCategory | null; articles: Article[] }> }> = {}
  
  items.value.forEach(article => {
    const categoryId = article.categoryId || 'no-category'
    const subCategoryId = article.subCategoryId || 'no-subcategory'
    
    if (!grouped[categoryId]) {
      const category = categories.value.find(c => c.id === article.categoryId) || null
      grouped[categoryId] = {
        category,
        subCategories: {}
      }
    ...
    if (!grouped[categoryId].subCategories[subCategoryId]) {
      const subCategory = subCategories.value.find(sc => sc.id === article.subCategoryId) || null
  ...
  Object.keys(grouped).forEach(categoryId => {
    Object.keys(grouped[categoryId].subCategories).forEach(subCategoryId => {
      grouped[categoryId].subCategories[subCategoryId].articles.sort((a, b) => {
        const dateA = new Date(a.publishedAt || a.createdAt).getTime()
        const dateB = new Date(b.publishedAt || b.createdAt).getTime()
        return dateB - dateA
      })
...
<template>
  <section class="container mx-auto px-4 py-10">
    ...
    <div class="mb-8">
      <h1 class="text-3xl font-bold mb-2 text-gray-900 dark:text-white">Artikel</h1>
      <p class="text-gray-600 dark:text-gray-400">Temukan artikel terbaru berdasarkan kategori dan sub-kategori</p>
    </div>

    <!-- Filter & Article List Container -->
    <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div class="p-6 md:p-8">
        ...
        <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-6">
          <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-2">
              ...
              Filter & Kelola Artikel
            </h2>
            <p class="text-sm text-gray-600 dark:text-gray-400">Pilih kategori atau sub-kategori untuk menyaring artikel</p>
          </div>
        ...
        <div v-if="selectedCategoryId || selectedSubCategoryId" class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <div class="mt-4 flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            Menampilkan <span class="font-semibold text-blue-600 dark:text-blue-400">{{ filteredArticles.length }}</span> dari <span class="font-semibold">{{ items.length }}</span> artikel
          </div>
        </div>
      </div>
      ...
      <div class="p-6 md:p-8">
        ...
        <div v-else-if="viewMode === 'grouped'">
          <div v-if="Object.keys(groupedArticles).length === 0" class="text-center py-20 text-gray-500 dark:text-gray-400">
            Belum ada artikel yang dikelompokkan
          </div>

          <div v-for="(group, categoryId) in groupedArticles" :key="categoryId" class="mb-12">
            <!-- Category Header -->
            <div class="mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
              <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                  ({{ Object.values(group.subCategories).reduce((sum, sub) => sum + sub.articles.length, 0) }} artikel)
                </span>
              </h2>
            </div>
            ...
            <div v-for="(subGroup, subCategoryId) in group.subCategories" :key="subCategoryId" class="mb-8">
              ...
              >
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                  ({{ subGroup.articles.length }} artikel)
                </span>
              </h3>
              ...
              <div class="grid md:grid-cols-3 gap-6">
                <article
                  ...
                  @click="goDetail(a.slug)"
                >
                  <img
                    v-if="a.imagePath"
                    :src="`${ARTICLE_IMAGE_BASE}${a.imagePath}`"
        ...
        <!-- All Articles View -->
        <div v-else>
          <div v-if="filteredArticles.length === 0" class="text-center py-20 text-gray-500 dark:text-gray-400">
            <p class="text-lg mb-2">Tidak ada artikel ditemukan</p>
            <button
              @click="clearFilter"
          ...
          <div v-else class="grid md:grid-cols-3 gap-6">
            ...
            >
              ...
              <div class="p-4">
                ...
                <div v-if="a.category || a.subCategory" class="flex flex-wrap gap-2 mb-3">
                  <span
                    v-if="a.category"
                ...
                <h2 class="font-semibold text-lg line-clamp-2 text-gray-900 dark:text-gray-100">{{ a.title }}</h2>
